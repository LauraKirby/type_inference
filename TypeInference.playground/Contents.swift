...
/*:
 ...
 EXPERIMENT - show how delcaring a type can be helpful.
 
 ![Experiment icon](experiment_sm.png)
 Let's say we would like store the year and month a new drought began in California.

 To keep our numbers in chronolocial order, we might store start date as 201301 or Double 2013.01, both of which could be interpreted to mean the 1st month in 2013. We can see how these two numbers may have equivalent meaning; however, they are different types. 
 */
...
/*:
 Further down in our source code, when we are ready to store the returned value or implement a variable which accesses that value, we may forget which type the function returns or the variable holds. To increase readablity, let's explicitly declare the type when the variable is created.
*/
